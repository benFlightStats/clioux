(ns clioux.handler
  (:require [compojure.core :refer [defroutes routes wrap-routes]]
            [clioux.routes.home :refer [home-routes]]
            
            [clioux.middleware :as middleware]
            [clioux.session :as session]
...
(defn init
  ...
  (timbre/set-config!
    ...
    {:path "clioux.log" :max-size (* 512 1024) :backlog 10})

  (if (env :dev) (parser/cache-off!))
  (start-nrepl)
  ;;start the expired session cleanup job
  (session/start-cleanup-job!)
  (timbre/info "\n-=[ clioux started successfully"
...
(defn destroy
  ...
  []
  (timbre/info "clioux is shutting down...")
  (stop-nrepl)
